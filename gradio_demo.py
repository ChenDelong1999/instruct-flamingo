...
class InferencerAPI:
    ...
    def __call__(self, 
        ...
        result = clever_flamingo_api(prompt, imgpaths)
        return result, '<API does not provide full text>'
            ...
            if message:
                if type(message) is tuple and message[
                    ...
                    context.append(self.sep +
                                #    role + ":\n" + message + self.sep + "<|endofchunk|><image>")
...
def bot(
    ...
    inference_results, full_texts = inferencer(inputs, image_paths, max_new_token,
                                   num_beams, temperature, top_k, top_p,
                                   do_sample, length_penalty)
    ...
    with gr.Blocks(title="Clever Flamingo🦩") as demo:
        ...
        with gr.Row():
            with gr.Column(scale=3):
                imagebox = gr.Image(type="filepath", shape=[336,336])
                with gr.Accordion(
                        "Parameters",
                        open=True,
                ):
